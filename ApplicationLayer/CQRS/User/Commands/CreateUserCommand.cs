...
{
    // 1. User Information
    [Required, RegularExpression(@"^[a-zA-Z\s]{8,40}$", ErrorMessage = "Name can only contain letters and spaces.")]
    ...
    [Required, RegularExpression(@"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{8,40}$", ErrorMessage = "Password must be at least 8 characters and contain at least one lowercase letter, one uppercase letter, one numeric digit, and one special character.")]
